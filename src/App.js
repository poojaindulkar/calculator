...
function reducer(state,{type,payload}){
  switch(type){
    case ACTIONS.ADD_DIGIT:
      ...
      return{
        ...state,// used to create a copy of the current state object.
...
const INTEGER_FORMARTTER =  new Intl.NumberFormat("en-us",{
  maximumFractionDigits:0
})
function formatOperand(operand){
  if(operand==null) return 
  const[integer,decimal]=operand.split(".")
  if(decimal==null)return INTEGER_FORMARTTER.format(integer)
  return `${INTEGER_FORMARTTER.format(integer)}.${decimal}`
}
function App() {
  const [{currentOperand, previousOperand, operation},dispatch]=useReducer(reducer,{});

  return (
    <div className='claulator-grid'>
